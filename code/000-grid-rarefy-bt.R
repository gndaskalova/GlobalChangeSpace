...
#### 0 - Import data =========================================================
bt_meta <-bioTIMEMetadataJune2021
bt_abu_bio <- BioTIMEQJune2021
...
# save(dgg, bt_grid_filtered, file='BioTIME_grid_filtered2022.Rdata')
...
# Create gridded file that has lat and long

simple_meta <- bt_meta %>% dplyr::select(BIOME_MAP, STUDY_ID) %>%
  distinct()
rare_samp_all <- left_join(rare_samp_all, simple_meta, by = "STUDY_ID")

bt_grid_coord <- left_join(rare_samp_all, rarefyID_cell_centre, by = "rarefyID")

bt_grid_coord <- bt_grid_coord %>% ungroup() %>%
  dplyr::select(-CLIMATE, -StudyMethod,
                -ABUNDANCE_TYPE, -BIOMASS_TYPE,
                -STUDY_ID.x, -STUDY_ID.y, -ObsEventID,
                -cell, -min_samp, 
                -Species, -Abundance, 
                -Biomass, -rarefy_resamp,
                -resample, -STUDY_ID.y, -cell_extent) %>%
  distinct()

bt_grid_coord <- bt_grid_coord %>% group_by(rarefyID) %>%
  mutate(startYear = min(YEAR),
         endYear = max(YEAR),
...
save(bt_grid_coord, file = "bt_grid_coord.RData")
